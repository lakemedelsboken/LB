...
try {
	config = require(path.join(process.cwd(), '/blog/config.jojo.json'));
} catch (e) {
}
...
function sendIndex(req, res, next) {
	...
	yoyo.readArticles(null, function (err, articles) {
		...
		renderObj = objExtend(renderObj, {'config': jojo.config});
...
Yoyo.prototype = {
	'readArticles': function (articleDir, callback) {
		...
		fs.readdir(articleDir, function (err, articles) {
			...
			articles = articles.filter(function(file) {return (file.indexOf(".txt") > 1);});
...
jojo.getUrl = function (article) {
	...
	urlParts.push(article.title.replace(/\s+/g, ' ').replace(/[^a-z0-9]/gi, '_'));
