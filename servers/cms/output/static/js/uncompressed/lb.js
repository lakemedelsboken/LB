...
(function(window, undefined){
	...
	lb = {
		...
		init: function() {
			...
			$("#toggleNavigation").on("click", function(event) {
				if ($(this).text().indexOf("Visa") > -1) {
					$("#sideMenu").show();
					$(this).html("<i class=\"fa fa-chevron-up\"></i> Göm meny");
				} else {
					$("#sideMenu").hide();
					$(this).html("<i class=\"fa fa-chevron-down\"></i> Visa meny");
				}
			});
		...
		activateScrollTarget: function(element) {
			...
			//Find the id in the menu
			var menu = $("ul.sideBarNavigation").first();

			if (menu.length === 1 && lb.currentId !== undefined) {
				var possibleItem = menu.find("." + lb.currentId.replace("#", ""));
				if (possibleItem.length === 1) {

					menu.menu("focus", null, possibleItem.parent());
		...
		initMenu: function(state, suppressSearch) {
			...
			self.addMenu("root", undefined, "forward", undefined, menuAnimationDuration, true, function() {
...
*/
		...
		setFullscreen: function(event) {
			if (!lb.fullScreen) {
				$("#fullscreenIcon").removeClass("fa-expand").addClass("fa-refresh");
				...
				setTimeout(function() {
					$("#fullscreenIcon").removeClass("fa-refresh").addClass("fa-expand");
				}, 200);
		...
		setNormalscreen: function(event) {
			...
			if (lb.fullScreen) {
				$("#normalscreenIcon").removeClass("fa-compress").addClass("fa-refresh");
				...
				setTimeout(function() {
					$("#normalscreenIcon").removeClass("fa-refresh").addClass("fa-compress");
				}, 200);
		...
		addMenu: function(menuId, firstText, direction, chapter, animDuration, skipFocus, next) {
			...
			if (self.cachedMenus[menuId] === undefined) {
				var newMenu = $("<ul id=\"menu_" + menuId + "\" class=\"sideBarNavigation\" />"); //nav nav-tabs nav-stacked

				newMenu.on('mousewheel DOMMouseScroll', function(e) {
					//Per: http://jsfiddle.net/rudiedirkx/F8qSq/light/
					if ($(this)[0].scrollHeight !== $(this).outerHeight() ) {
						var d = e.originalEvent.wheelDelta || -e.originalEvent.detail;
						var dir = d > 0 ? 'up' : 'down';
						var stop = (dir == 'up' && this.scrollTop == 0) || (dir == 'down' && this.scrollTop == this.scrollHeight-this.offsetHeight);
				...
				if (chapter !== undefined) {
					newMenu.append($("<li class=\"backItem\"><a href=\"/" + (chapter !== undefined ? chapter : "") + "#back\" class=\"btn menuBack\"><i class=\"fa fa-angle-up\"></i> Tillbaka</a></li>"));
				}
				...
				newMenu.menu(
					...
					if (openMenuId === "back") {
						...
						if (anchor.hasClass("background")) {
							...
							if (false && currentChapter.indexOf(chapter) > -1) {
								//Item is currently open chapter
								...
								//document.location.href = chapter;
				...
				lb.getTocMenuItems(menuId, function(err, data) {
					if (err) {
						...
						newMenu.append($("<li><a href=\"#\"><i class=\"fa fa-info-circle\"></i> Ett fel inträffade, lyckades inte hämta innehållsförteckningen</a></li>"));
						//Refresh
						...
						*/
						...
						for (var i=0; i < data.length; i++) {
							...
							if (item.headeritem !== undefined) {
								...
								if (titleItem.length === 1) {
									titleItem.attr("href", "" + firstItem.chapter + id);
									titleItem.text(item.title)
								} else {
									newMenu.append($("<li><a class=\"titleItem " + id + "\" href=\"" + item.chapter + id + "\"" + ((item.chapter !== undefined) ? " data-chapter=\"" + item.chapter + "\"" : " data-chapter=\"\"") + " data-has-children=\"false\">" + item.title + "</a></li>"));
								}
...
/*
								...
								if (indentation > 0) {
									indentation = "style=\"padding-left: " + indentation + "px; font-size: " + (15 - item.indent) + "px;\" ";
								} else {
									indentation = "";
								}
*/
		...
		getTocMenuItems: function(parentId, callback) {
			...
			function handleData(menu, indentation) {
				...
				menu.children().each(function(index, item) {
					...
					if (item.find("ul").length > 0) {
						...
						var firstContent = item.contents().first();
						var secondContent = firstContent[0].nextSibling.nodeValue;

						if (firstContent.prop("tagName") === "I") {
							resultItem.icon = firstContent.attr("class");
						}

						resultItem.title = $.trim(item.text());
						//resultItem.id = resultItem.title;

						var anchor = item.find("a").first();
						if (anchor.length === 1) {

							if (firstContent.prop("tagName") !== "I") {
								var possibleIcon = anchor.find("i").first();
								if (possibleIcon.length === 1) {
									resultItem.icon = possibleIcon.attr("class");
								}
							}

							resultItem.title = $.trim(anchor.text());
							if (secondContent !== null) {
								resultItem.title = $.trim(secondContent)
							}
							resultItem.chapter = anchor.attr("href");
							//resultItem.id = resultItem.chapter;

							if (resultItem.chapter.indexOf("#") > -1) {
								resultItem.id = resultItem.chapter.split("#")[1];
								resultItem.chapter = resultItem.chapter.split("#")[0];
							}

//							if (resultItem.id.indexOf("#") > -1) {
//								resultItem.id = resultItem.id.split("#")[1];
//							}
						}

						resultItem.hasChildren = false;
						resultItem.type = "division";

						results.push(resultItem)

						handleData(subMenu, indentation + 1);
						...
						if (firstContent[0].nextSibling) {
							secondContent = firstContent[0].nextSibling.nodeValue;
						}
					...
					//	console.log("Could not handle item");
					//	console.log(item);
					//}
				});
			...
			if (parentId === "root") {
				...
				var menu = $("#sideBar").first();

				//Check if first item refers to root


				if (menu.length === 1) {

					var firstItem = menu.find("a").first();
					if (firstItem.length === 1 && firstItem.attr("href") === "/") {
						//Find the next menu
					...
					return callback("Kunde inte hitta rubriker");
				}

			} else {

				callback("Något gick fel");
...
*/
		...
		initKeyboardNavigation: function() {
			...
			$("#medList").on('mousewheel DOMMouseScroll', function(e) {
				//Per: http://jsfiddle.net/rudiedirkx/F8qSq/light/
				if ($(this)[0].scrollHeight !== $(this).outerHeight() ) {
					var d = e.originalEvent.wheelDelta || -e.originalEvent.detail;
					var dir = d > 0 ? 'up' : 'down';
			...
			$("#medDisplay").on('mousewheel DOMMouseScroll', function(e) {
				//Per: http://jsfiddle.net/rudiedirkx/F8qSq/light/
				if ($(this)[0].scrollHeight !== $(this).outerHeight() ) {
					var d = e.originalEvent.wheelDelta || -e.originalEvent.detail;
					var dir = d > 0 ? 'up' : 'down';
		...
		openPage: function(chapter, id, forceMenuUpdate) {
			...
			var append = [];
			if ($("#search").val() !== "") {
				append.push("search=" + encodeURIComponent($("#search").val()));
			}

			if (id !== "") {
				append.push("id=" + id + "#" + id);
			}

			if (append.length > 0) {
				document.location.href = chapter + "?" + append.join("&");
			} else {
				document.location.href = chapter;
			}
...
//				$(document).bind("keydown", "1 2 3 4 5 6 7 8 9 0 a b c d e f g h i j k l m n o p q r s t u v w x y z å ä ö", function(event) {
				...
				lb.medicineSearch = $.getJSON("/medicinesearch?search=" + encodeURIComponent(searchValue), function(results) {
					...
					if ($.trim(search.val()) === searchValue) {
						...
						for (var i=0; i < results.length; i++) {
							var item = results[i];
							if (item.type === "atc") {
								resultsList.append($("<li id=\"searchResults_" + item.id + "\"><a href=\"/atc/" + item.id + "\" class=\"atcCodeInPopover searchResult\" data-indentation=\"0\" data-atcid=\"" + item.id + "\" data-atctitles=\"" + item.titlePath.replace(/\s\/\s/g, "--").replace(/\s/g, "_") + "\"><i class=\"fa fa-plus-square\"></i> <strong>" + item.title_HL + "</strong><br><small>" + item.titlePath_HL + "</small></a></li>"));
				...
				lb.contentSearch = $.getJSON("/contentsearch?search=" + encodeURIComponent(searchValue), function(results) {
					...
					if ($.trim(search.val()) === searchValue) {
						...
						for (var i=0; i < results.length; i++) {
							...
							resultsList.append($("<li><a class=\"searchResult\" href=\"" + contentItem.url + "?id=" + contentItem.id + "#" + contentItem.id + "\"><i class=\"fa " + lb.getIcon(contentItem.type) + "\"></i> <strong>" + title + "</strong><br><small>" + titlePath + "</small><div>" + contentItem.content_HL + "</div></a></li>"));
						}
						if (results.length === 0) {
		...
		handleAtcCodeInPopover: function(event) {
			...
			if (currentAnchor.attr("data-already-loaded") !== undefined) {
				...
				//Set icon
				currentAnchor.find("i.fa-minus-square").removeClass("fa-minus-square").addClass("fa-plus-square");
				...
				currentAnchor.find("i.fa-plus-square").removeClass("fa-plus-square").addClass("fa-refresh");
				...
				lb.getATCItems(atcCode, function(err, results) {
					currentAnchor.find("i.fa-refresh").removeClass("fa-refresh").addClass("fa-minus-square");
					if (err) {
						currentAnchor.find("i.fa-minus-square").removeClass("fa-minus-square").addClass("fa-info-circle");
						...
						for (var i = 0; i < results.data.length; i++) {
							...
							if (item.hasChildren) {
								content += "<li id=\"" + tipId + "_" + item.id + "\"><a href=\"/atc/" + item.id + "\" data-indentation=\"" + (indentationLevel + 1) + "\" class=\"atcCodeInPopover\" style=\"padding-left: " + indentationPixels + ";\"><i class=\"fa fa-plus-square\"></i><i class=\"fa fa-angle-down pull-right\"></i> <strong>" + item.text + "</strong></a></li>";
						...
						if (content !== "") {
							...
							currentListItem.after("<li><a style=\"padding-left: " + indentationPixels + ";\"><i class=\"fa fa-info-circle\"></i> Det finns inga underliggande preparat</a></li>");
						}
						parentMenu.menu("refresh");
		...
		handleGenericas: function(event) {
			...
			if ($(this).attr("data-original-title") === undefined) {
				...
				for (var i=0; i < atcCodes.length; i++) {
					...
					content += "<h6 class=\"" + atcCode + "\">" + atcTitles[i].replace(/\-\-/g, " - ").replace(/_/g, " ") + "</h6><ul class=\"" + atcCode + " nav nav-tabs nav-stacked\"><li class=\"loading\"><a href=\"#\"><i class=\"fa fa-refresh fa-spin\"></i> Hämtar rubriker...</a></ul>";
				}
						...
						for (var i=0; i < atcCodes.length; i++) {
							...
							*/
							...
							lb.getATCItems(atcCode, function(err, results) {
								...
								if (err) {
									currentATCMenu.append("<li><a href=\"#\"><i class=\"fa fa-info-circle\"></i> Kunde inte hitta rubriker</a></li>");
									...
									for (var i = 0; i < results.data.length; i++) {
										...
										var item = results.data[i];
										if (item.hasChildren) {
											currentATCMenu.append("<li id=\"" + tipId + "_" + item.id + "\"><a href=\"/atc/" + item.id + "\" data-indentation=\"0\" class=\"atcCodeInPopover\"><i class=\"fa fa-plus-square\"></i><i class=\"fa fa-angle-down pull-right\"></i> <strong>" + item.text + "</strong></a></li>");
										} else if (item.children && item.children.length > 0) {
											for (var j = 0; j < item.children.length; j++) {
												var productItem = item.children[j];
												var images = "";
							...
							function onCompletedAtcCodes() {
								...
								if ($("#" + tipId).find("h6").length === 0) {
									$("#" + tipId).find(".popover-content").html("<h5><i class=\"fa fa-info-circle\"></i> Tyvärr hittades inga preparat för denna rubrik</h5>");
								}
							}
						}
		...
		handleBoxSearch: function(event) {
			...
			content += "<h6 class=\"" + className + "\">" + name + "</h6><ul class=\"" + className + " nav nav-tabs nav-stacked\"><li class=\"loading\"><a href=\"#\"><i class=\"fa fa-refresh fa-spin\"></i> Hämtar informationsrutor...</a></ul>";
			...
			$(this).clickover({
				...
				onShown: function() {
					...
					$.getJSON("/boxsearch?search=" + encodeURIComponent(searchName), function(results) {
						...
						if (results.length === 0) {
							currentTherapyMenu.append("<li><a href=\"#\"><i class=\"fa fa-info-circle\"></i> Preparatet nämns inte i några informationsrutor</a></li>");
							...
							for (var i = 0; i < results.length; i++) {
								...
								currentTherapyMenu.append($("<li><a class=\"searchResult\" href=\"" + item.url + "#" + item.id + "\"><i class=\"fa " + lb.getIcon(item.type) + "\"></i> <strong>" + title + "</strong><br><small>" + titlePath + "</small><div>" + item.content_HL + "</div></a></li>"));
						...
						currentTherapyMenu.append($("<li><a href=\"#\"><i class=\"fa fa-info-circle\"></i> Sökningen misslyckades<br><small>Var god försök igen lite senare</small></a></li>"));
						currentTherapyMenu.menu("refresh");
					});
		...
		handleProduct: function(event, forcedNplId) {
			...
			categoryTitle.html("<i class=\"fa fa-refresh fa-spin\"></i> Hämtar produktinformation...</a>");

			$("#medListToggler").html("<i class=\"fa fa-refresh fa-spin\"></i> Hämtar...");
			$("#medListToggler").attr("data-open", "false");

			body.empty();
			body.html("<div class=\"hero-unit\"><h5><i class=\"fa fa-refresh fa-spin\"></i> Hämtar produktinformation...</h5></div>");
			list.empty();
			list.append("<li><a href=\"#\"><i class=\"fa fa-refresh fa-spin\"></i> Hämtar lista...</a></li>");
			list.menu("refresh");
			...
			$.getJSON("/medlist?id=" + nplId, function(medList) {
				...
				if (medList.length > 0) {
					...
					for (var i=0; i < medList.length; i++) {
						...
						if (item.type === "product") {
							...
							categoryTitle.html("<i class=\"fa fa-info-circle\"></i> " + item.title + "</a>");
						}
					}
				...
				if (products.length === 1) {
					$("#medListToggler").html("<i class=\"fa fa-info-circle\"></i> Det finns inga liknande preparat.");
				} else if (products.length > 1) {
					$("#medListToggler").html("<i class=\"fa fa-info-circle\"></i> Visa " + (products.length - 1) + " liknande preparat...");
				}
			...
			$.getJSON("/products/" + nplId + ".json", function(product) {
				...
				if (product["noinfo"] !== undefined && product["noinfo"] === true) {
					...
					body.html("<div class=\"hero-unit\"><h5><i class=\"fa fa-info-circle\"></i> Det finns tyvärr ingen förskrivarinformation i Fass om detta läkemedel.</h5><h5><a href=\"http://www.fass.se/LIF/product?4&userType=0&nplId=" + nplId + "\" target=\"_blank\"><i class=\"fa fa-search\"></i> Visa preparat på Fass.se</a></h5</div>");
				} else {
		...
		renderProductInfo: function(product, nplId, container, forcedNplId) {
			...
			if (!product.provider || product.provider === "fass") {
				...
				if (providerLink !== undefined) {
					...
					medInfo.append($("<div class=\"pull-right\" style=\"width: 120px;\">" + providerImage + "</div>"));
				}
			...
			if (product.spcLink && product.spcLink !== "") {
				...
				medInfo.append($("<div><strong>Produktresumé:</strong> <a target=\"_blank\" href=\"" + product.spcLink + "\">" + linkText + " <i class=\"fa fa-file-text-alt\"></i></a></div>"));
			...
			*/
			...
			for(headLine in product.sections) {
				...
				if (content !== "" && content !== "<p></p>") {
					medHeadLines.append($("<div class=\"accordion-group\"><div class=\"accordion-heading\"><a class=\"accordion-toggle\" data-toggle=\"collapse\" href=\"#" + headLineId + "\"><i class=\"fa fa-plus-square leftIcon\"></i><i class=\"fa fa-angle-down pull-right rightIcon\"></i> " + headLine + "</a></div><div id=\"" + headLineId + "\" class=\"accordion-body collapse\"><div class=\"accordion-inner\">" + content + "</div></div></div>"));
				}
			}
			*/
			//Toggle all
			...
			if(product.noSections === undefined || products.noSections === false){
				body.append($("<div><a href=\"#\" id=\"toggleAllSections\" class=\"btn btn-primary btn-block\"><i class=\"fa fa-plus\"></i> <span>Öppna alla rubriker</span></a></div>"));
			}
			...
			for(headLine in product.sections) {
				...
				var content = product.sections[headLine];
				if (content !== "" && content !== "<p></p>") {
					medHeadLines.append($("<h4 class=\"section-header\"><a class=\"toggleSection\" href=\"#" + headLineId + "\"><i class=\"fa fa-plus-square leftIcon\"></i><i class=\"fa fa-angle-down pull-right rightIcon\"></i> " + headLine + "</a></h4><div id=\"" + headLineId + "\" class=\"section\">" + content + "</div>"));
				}
			}
		...
		toggleAccordion: function(event) {
			...
			if ($(this).text().indexOf("Öppna alla") > -1) {
				...
				$("i.leftIcon", "#medHeadLines").removeClass("fa-plus-square").addClass("fa-minus-square");
				$("i.rightIcon", "#medHeadLines").removeClass("fa-angle-down").addClass("fa-angle-up");
				$(this).find(".fa").removeClass("fa-plus").addClass("fa-minus");
				$(this).find("span").text("Stäng alla rubriker");
			} else {
				$(".section", "#medHeadLines").hide();
				$("i.leftIcon", "#medHeadLines").removeClass("fa-minus-square").addClass("fa-plus-square");
				$("i.rightIcon", "#medHeadLines").removeClass("fa-angle-up").addClass("fa-angle-down");
		...
		toggleSection: function(event) {
			...
			var isOpen = $(".leftIcon", toggler).first().hasClass("fa-minus-square");

			if (isOpen) {
				$("i.leftIcon", toggler).removeClass("fa-minus-square").addClass("fa-plus-square");
				$("i.rightIcon", toggler).removeClass("fa-angle-up").addClass("fa-angle-down");
				section.hide();
			} else {
				$("i.leftIcon", toggler).removeClass("fa-plus-square").addClass("fa-minus-square");
				$("i.rightIcon", toggler).removeClass("fa-angle-down").addClass("fa-angle-up");
				section.show();
			}
...
function doHighlight(bodyText, searchTerm, highlightStartTag, highlightEndTag)
			{
					...
					if ((!highlightStartTag) || (!highlightEndTag)) {
							highlightStartTag = "<font style=' background-color: #ADD8E6;'>";
			...
			function highlightSearchTerms(searchText, treatAsPhrase, warnOnFailure, highlightStartTag, highlightEndTag)
			{
					...
					var bodyText = $('#mainContainer').html();
					for (var i = 0; i < searchArray.length; i++) {
							bodyText = doHighlight(bodyText, searchArray[i], highlightStartTag, highlightEndTag);
					}

					$('#mainContainer').html(bodyText);
					return true;
			}
			var arr = [];
			var ter=$('#mainContainer').html();
			arr.push( ter);
			if(getUrlParameter('search')!=null){
					id=getUrlParameter('search');
