...
QuerySuggester.prototype.getSuggestionUrl = function(query) {
    var list = document.getElementById("search_form_scope");
    if (list){
        var index = list.selectedIndex;
        var category = '&c='+list[index].value;
    }else
        category='';

    return this.__suggestionUrl + query +category;
};

/**

 */
//QuerySuggester.prototype.setClass = function(class) {
//    this.__class = class;
//};
...
QuerySuggester.prototype.fetchAndDisplaySuggestions = function() {
    ...
    /* Do not cache result. Gives inconsistency when using different dictionaries
    if (this.__cache[val]) {
...
QuerySuggester.prototype.buildSuggestionsHtml = function(matches) {
    ...
    for (var i = 0; i < length; ++i) {
        dispMatch = matches[i * 2 + 3] != '' ? matches[i * 2 + 3] : matches[i * 2 + 2]; //If metadata is empty, use base
